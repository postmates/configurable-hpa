...
/*
* copy of ./pkg/controller/autoscaler/replica_calculator_test.go
* from kubernetes-1.10.8
...
func (tc *replicaCalcTestCase) prepareTestClient(t *testing.T) (*fake.Clientset, *metricsfake.Clientset, *cmfake.FakeCustomMetricsClient, *emfake.FakeExternalMetricsClient) {
	...
	fakeClient.AddReactor("list", "pods", func(action core.Action) (handled bool, ret runtime.Object, err error) {
		fmt.Printf("fakeClient.AddReactor list pods: %v\n", action)
		obj := &v1.PodList{}
	...
	fakeMetricsClient.AddReactor("list", "pods", func(action core.Action) (handled bool, ret runtime.Object, err error) {
		fmt.Printf("fakeMetricsClient.AddReactor list pods: %v\n", action)
		if tc.resource != nil {
	...
	fakeCMClient.AddReactor("get", "*", func(action core.Action) (handled bool, ret runtime.Object, err error) {
		fmt.Printf("fakeCMClient.AddReactor get *: %v\n", action)
		...
		mapper := testrestmapper.TestOnlyStaticRESTMapper(runtime.NewScheme())
		metrics := &cmapi.MetricValueList{}
		...
		groupResource := schema.GroupResource{Group: mapping.GroupVersionKind.Group, Resource: mapping.Resource.String()}
...
/*
* TODO: this test fails, though in vanilla k8s it doesn't
* diagnostics:
* --- FAIL: TestReplicaCalcScaleUpCMObject (0.00s)
    require.go:794:
        	Error Trace:	replica_calculator_test.go:351
        	            				replica_calculator_test.go:506
        	Error:      	Received unexpected error:
        	            	unable to get metric qps: Deployment on test-namespace some-deployment/unable to fetch metrics from custom metrics API: unable to get mapping for Deployment.extensions: no matches for kind "Deployment" in group "extensions"
        	Test:       	TestReplicaCalcScaleUpCMObject
        	Messages:   	there should not have been an error calculating the replica count
...
* TODO: this test fails, though in vanilla k8s it doesn't
* diagnostics:
* --- FAIL: TestReplicaCalcScaleDownCMObject (0.00s)
    require.go:794:
        	Error Trace:	replica_calculator_test.go:351
        	            				replica_calculator_test.go:600
        	Error:      	Received unexpected error:
        	            	unable to get metric qps: Deployment on test-namespace some-deployment/unable to fetch metrics from custom metrics API: unable to get mapping for Deployment.extensions: no matches for kind "Deployment" in group "extensions"
        	Test:       	TestReplicaCalcScaleDownCMObject
        	Messages:   	there should not have been an error calculating the replica count
...
* diagnostics:
*
--- FAIL: TestReplicaCalcToleranceCMObject (0.00s)
    require.go:794:
        	Error Trace:	replica_calculator_test.go:351
        	            				replica_calculator_test.go:717
        	Error:      	Received unexpected error:
        	            	unable to get metric qps: Deployment on test-namespace some-deployment/unable to fetch metrics from custom metrics API: unable to get mapping for Deployment.extensions: no matches for kind "Deployment" in group "extensions"
        	Test:       	TestReplicaCalcToleranceCMObject
        	Messages:   	there should not have been an error calculating the replica count
